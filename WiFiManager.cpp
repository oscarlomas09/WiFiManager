...
void WiFiManager::setupConfigPortal()
{
  ...
  server->on(String(F("/wifiPost")), std::bind(&WiFiManager::handleWifiSavePost, this));
...
void WiFiManager::handleRoot()
{
  ...
  page.replace("{v}", _apName); // page title to Access Point Name
  page += FPSTR(HTTP_SCRIPT);
  page += FPSTR(HTTP_STYLE);
  page += _customHeadElement;
  page += FPSTR(HTTP_HEAD_END);
  page += String(F("<h3>WiFi Configuration</h3>"));
...
/** Handle the WLAN save form and redirect to WLAN config page again */
void WiFiManager::handleWifiSavePost()
{
  DEBUG_WM(F("WiFi save POST"));
